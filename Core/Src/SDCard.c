...
//#define SDCARD_CS_Pin        GPIO_PIN_2 	//  D2 ?
...
/////////////////////// Инициализация и проверка SDCard //////////////////////

//SDCard Init 12/03/25
uint8_t SD_Code = 0;
...
void initSD() {
    ...
    uint32_t startBlockAddr = 0x00ABCD; // запись аварий начнем с 32-го блока
    ...
    memset(block, 0, sizeof(block)); //??? треб-т: string.h
    ...
    if(code < 0) {
        SD_Code = code;	//UART_Printf("SDCARD_WriteBegin() failed: code = %d\r\n", code);
        return;
    }
    ...
    for(int i = 0; i < 3; i++) {
        ...
        if(code < 0) {
            SD_Code = code; //UART_Printf("SDCARD_WriteData() failed: code = %d\r\n", code);
            return;
        }
    ...
    //if(code < 0) { UART_Printf("SDCARD_WriteEnd() failed: code = %d\r\n", code); return;}
    ...
    if(code < 0) {
        SD_Code = code;//UART_Printf("SDCARD_ReadBegin() failed: code = %d\r\n", code);
        return;
    }
    ...
    for(int i = 0; i < 3; i++) {
        ...
        if(code < 0) {
            SD_Code = code; //UART_Printf("SDCARD_ReadData() failed: code = %d\r\n", code);
            return;
        }
